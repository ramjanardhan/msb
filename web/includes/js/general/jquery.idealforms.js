 ...
 */
        module.exports = {
            ...
            loginId: 'Must be a valid LoginId',
            ...
            firstName: 'Must be at least 3 characters long and must only contain letters',
            lastName: 'Must be at least 3 characters long and must only contain letters',
            equalto: 'Must have the same value as the "{0}" field',
            maxsize:'Size must be lest than 2MB',
                ...
                _buildDatepicker: function() {
                    ...
                    if (jQuery.ui && $datepicker.length) {
                        ...
                        $datepicker.each(function() {
                            ...
                            $(this).datepicker({
                                defaultDate: '1/1/1930',
                               // yearRange: '1930:2014',
                               yearRange: '1930:' + year + '', defaultYear: d.getMonth()+d.getDay()+year,
                                changeMonth:true,
                                changeYear:true,
        ...
        module.exports = {
            ...
            _i18n: function() {
                ...
                $.each($.idealforms.i18n, function(locale, lang) {
                    ...
                    for (var ext in lang) options[ext] = {
                        i18n: lang[ext]
                    };
            ...
            _addEvents: function(input) {
                ...
                var self = this
                , $field = this._getField(input);
               // alert("this.name"+self);
                $(input)
                .on('change keyup', function(e) {
                    // alert("on change key up-->"+this.name); //Nagireddy
                         ...
                         if(input.value!=null){
                            ...
                            // alert("ext-->"+extension);
                             ...
                             if(extension=="jpg"|| extension=="png"||extension=="jpeg"){
                                ...
                                // alert("size-->"+imgSize);
                                 if(parseInt(imgSize)<2000000) {
                                     //alert("correct mb");
                                      $field.find(self.opts.error).hide();
                                      self._validate(this, true, true); 
                                 }else{
                                  input.value="";  //12345
                                  self._handleError(input,true,true);
                                  self._validate(input, true, true); 
                                  self._handleError(input, 'File size should be less than 2 MB.', false);
                                 }
                             }else{
                                // alert("in ext fail");
                                  input.value="";  //12345
                                  self._handleError(input,true,true);
                                  self._validate(input, true, true); 
                                  self._handleError(input, 'Allowed Extenssions : .jpg , .png, .jpeg', false);
                             }
                         }
                         
                     } else if(this.name=='loginId'){
                       // alert("in if");
                        self._handleError(input, 'Enter Valid LoginId!', false);
                         if(ValidateEmail(this.value)){
                             $field.find(self.opts.error).hide();
                            // $field.find(self.opts.error).show();
                            self. _Customvalidate(this);
                         }
                    }else{
                       self._validate(this, true, true); 
                    }
            ...
            // added by nag..
            _Customvalidate: function(input){
                
                var self=this, $field = this._getField(input);
                $field.addClass('ajax');
                 $.ajax({
                            type: 'POST',
                            url:'loginIdCheckservice.action?loginId='+input.value,
                            success: function(data, textStatus, request){
                              ...
                              if(obj.message=='valid'){
                                ...
                                self._validate(input, true, true); 
                                  ...
                                  self._handleError(input,true,true);
                                  self._validate(input, true, true); 
                                  self._handleError(input, 'LoginId is already existed!please try with another LoginId', false);
                              }
                            ...
                            error: function (request, textStatus, errorThrown) {
                                ...
                                self._handleError(input,true,true);
                                self._validate(input, true, true); 
                              self._handleError(input, 'Please try later!', false);
                            }
                        }); 
             ...
             /*   var self = this
                ...
                , oldValue = $field.data(inputvales)
                , valid = true
                , rule; */
            },
 ...
 */
        module.exports = {
            ...
            lastName:/^[a-z](?=[\w.]{3,31}$)\w*\.?\w*$/i,
            address:/^[A-Za-z0-9 -]*[A-Za-z0-9][A-Za-z0-9 -]*$/,
            city:/^[a-z](?=[\w.]{2,30}$)\w*\.?\w*$/i,
            state:/^[a-z](?=[\w.]{2,11}$)\w*\.?\w*$/i,
